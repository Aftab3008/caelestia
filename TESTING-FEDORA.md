...
git clone https://github.com/YOUR_USERNAME/caelestia.git ~/.local/share/caelestia
...
2. **Enable 9 COPR repositories** (quickshell, lazydisk, jetbrains-mono-nerd, caskaydia-cove-nerd, material-symbols, libcava, app2unit, gpu-screen-recorder, hyprland)
3. **Enable RPM Fusion** (free + nonfree)
4. **Install ~50+ packages via dnf**
...
7. **Install manual packages** — dart-sass (npm), materialyoucolor (pip), papirus-folders (git script), darkly Qt style (cmake build)
...
> **Expected time:** 15-30 minutes depending on internet speed and CPU.
...
| **libcava** | From `zawertun/scrapyard` COPR. Check for F44 build; if missing, build from [cava source](https://github.com/karlstav/cava) with `-DBUILD_SHARED_LIBS=ON`. |
| **papirus-folders** | No Fedora package. Installer builds from GitHub. |
| **darkly Qt style** | Built from source by installer. Needs `cmake`, `extra-cmake-modules`, `kf6-kdecoration-devel`, `kf6-kirigami-devel`. |
